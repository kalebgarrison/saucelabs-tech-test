...
describe('Working Sauce', function () {
    ...
    // Task II
    let input = driver.findElement(By.name('i_am_a_textbox'));     

    await input.clear();
    await input.sendKeys('Sauce');

    // Task III
    let email = driver.findElement(By.name("fbemail"));
    let comments = driver.findElement(By.name("comments"));

    await email.clear();
    await email.sendKeys('kalebgarrison@gmail.com');
    await comments.clear()
    await comments.sendKeys('Hi! This is a test');
